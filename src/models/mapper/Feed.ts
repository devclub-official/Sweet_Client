...
export const contentDtoToDomain = (dto: ContentDto[]): FeedSummary[] => {
  return dto.map(
    (item: ContentDto): FeedSummary => ({
      ...
      imageUrls: item.imageUrls.map(
        (image: string) => `${Config.MAIN_API_ORIGIN}${image}`,
      ),
...
export const getFeedDetailDtoToDomain = (
  dto: GetFeedDetailDto,
): FeedDetail => ({
  id: dto.id,
